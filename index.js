...
const { color, bgcolor } = require('./VILLCOMPUTER/color')
const { help } = require('./VILLCOMPUTER/help')
const kagApi = require('@kagchi/kag-api')
const { donasi } = require('./VILLCOMPUTER/donasi')
const { fetchJson } = require('./VILLCOMPUTER/fetcher')
const { recognize } = require('./VILLCOMPUTER/ocr')
const { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./VILLCOMPUTER/functions')
const tiktod = require('tiktok-scraper')
const ffmpeg = require('fluent-ffmpeg')
const { removeBackgroundFromImageFile } = require('remove.bg')
const welkom = JSON.parse(fs.readFileSync('./VILLCOMPUTER/welkom.json'))
const nsfw = JSON.parse(fs.readFileSync('./VILLCOMPUTER/nsfw.json'))
const samih = JSON.parse(fs.readFileSync('./VILLCOMPUTER/simi.json'))
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN?
            + 'VERSION:3.0\n' // NGAPAIN LAGI KALO GA MAU NUMPANG NAMA DOANG XIXIXIXI
            + 'FN:CO VILLCOMPUTER\n' // MENDING LU TOBAT SU!
            + 'ORG:Creator VILLCOMPUTER;\n' // KASIH CREDITS GUA SU!!!
...
const config = {
    A187: '💻VILLCOMPUTER💻', // TOBAT SU ASU
    instagram: 'https://instagram.com/villosmile_', // INFO JANGAN DI UBAH
    nomer: 'wa.me/6283843251769', // INFO SU JNGAN DI UBAH
    youtube: 'https://youtube.com/channel/BLUM PUNYA', // KINTIL
    whatsapp: 'https://wa.me/qr/CBONDIDGJ5SFO1', // BABI
...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code is ready, follow ig villosmile_`)
})
...
// client.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log(`${time}: Bot by ig:@villosmile_`)
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6283843251769@s.whatsapp.net"] // ganti nomer lu
			...
			switch(command) {
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`teksnya mana? contoh ${prefix}logowolf VILLO|Ganss`)
					reply(mess.wait)
           ...
           case 'demote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('𝐓𝐚𝐠 lah TOD 𝐭𝐚𝐫𝐠𝐞𝐭 𝐲𝐚𝐧𝐠 𝐦𝐚𝐮 𝐝𝐢 𝐭𝐮𝐫𝐮𝐧𝐤𝐚𝐧 𝐚𝐝𝐦𝐢𝐧')
              ...
              case 'abangjago':
              case 'abgjago':
                    let eerrr = fs.readFileSync('./VILLCOMPUTER/abangjago.mp3')
